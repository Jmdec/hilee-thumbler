...
export async function sendVerificationEmail(email: string, name: string, verificationToken: string) {
  ...
  const mailOptions = {
    from: process.env.SMTP_FROM || `"Hilee Tumbler" <${process.env.SMTP_USER}>`,
    to: email,
    subject: "Verify Your Email - Hilee Tumbler",
    html: `
      <!DOCTYPE html>
      <html>
        <head>
      ...
      .footer {
        ...
        <body>
          <div class="container">
            <div class="brand">Hilee Tumbler</div>
            <div class="content">
              <h2>Welcome, ${name}!</h2>
              <p>Thank you for registering with <strong>Hilee</strong>.</p>
