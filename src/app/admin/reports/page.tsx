...
interface StatusStats {
  ...
  preparing: StatusCount
  ready: StatusCount
  delivered: StatusCount
...
const defaultOrderStats = (): StatusStats => ({
  pending: DEFAULT_STATUS_COUNT,
  confirmed: DEFAULT_STATUS_COUNT,
  preparing: DEFAULT_STATUS_COUNT,
  ready: DEFAULT_STATUS_COUNT,
  delivered: DEFAULT_STATUS_COUNT,
  cancelled: DEFAULT_STATUS_COUNT,
  total: 0,
})
...
export default function ReportsPage() {
  ...
  const fetchReports = async () => {
    try {
      ...
      console.log("[reports] raw API result:", result)
      ...
      console.log("[reports] orders length:", orders.length)

      // --- Transform orders into ReportData ---
      const totalRevenue = orders.reduce((sum, o) => sum + Number(o.total), 0)
      const totalOrders = orders.length
      const newCustomers = new Set(orders.map((o) => o.customer_email)).size
      const avgOrderValue = totalOrders ? totalRevenue / totalOrders : 0

      const revenue_chart: ChartPoint[] = orders.map((o) => ({
        period: o.created_at.split("T")[0],
        revenue: Number(o.total),
        orders: 1,
      }))

      const top_productsMap: Record<number, TopProduct> = {}
      orders.forEach((o) =>
        o.order_items.forEach((item) => {
          if (!item.product) return
          const p = item.product
          if (!top_productsMap[p.id]) top_productsMap[p.id] = { product_id: p.id, name: p.name, total_qty: 0, total_revenue: 0 }
          top_productsMap[p.id].total_qty += item.quantity
          top_productsMap[p.id].total_revenue += Number(item.price) * item.quantity
        }),
      )
      const top_products = Object.values(top_productsMap)

      const statuses = ["pending", "confirmed", "preparing", "ready", "delivered", "cancelled"]
      const order_stats: StatusStats = { total: totalOrders } as any
      statuses.forEach((s) => {
        const count = orders.filter((o) => o.status === s).length
        order_stats[s] = { count, rate: totalOrders ? (count / totalOrders) * 100 : 0 }
      })

      const paymentMap: Record<string, { count: number; revenue: number }> = {}
      orders.forEach((o) => {
        if (!paymentMap[o.payment_method]) paymentMap[o.payment_method] = { count: 0, revenue: 0 }
        paymentMap[o.payment_method].count += 1
        paymentMap[o.payment_method].revenue += Number(o.total)
      })
      const payment_breakdown: PaymentRow[] = Object.entries(paymentMap).map(([method, val]) => ({
        method,
        payment_status: "",
        count: val.count,
        revenue: val.revenue,
      ...
      const reportData: ReportData = {
        summary: { total_revenue: totalRevenue, total_orders: totalOrders, new_customers: newCustomers, avg_order_value: avgOrderValue },
        revenue_chart,
        top_products,
        order_stats,
        payment_breakdown,
      ...
      console.log("[reports] transformed ReportData:", reportData)
      setData(reportData)
    } catch (err: any) {
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {summaryCards.map((card) => {
                      ...
                      return (
                        <div
                          key={card.title}
                          className="rounded-2xl bg-white/90 p-5 shadow-lg border flex items-center gap-4"
                            ...
                            {revenueChart.map((pt, i) => {
                              ...
                              return (
                                <div key={i} className="flex-1 flex flex-col items-center gap-1 group">
                                  ...
                                  <div
                                    className="w-full rounded-t-lg transition-opacity duration-150 group-hover:opacity-70 cursor-pointer"
                                    style={{ height: `${pct}%`, background: purpleGrad }}
                                    title={`${label}: ${fmt(pt.revenue)}`}
                                  />
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                        {Object.entries(STATUS_META).map(([key, meta]) => {
                          ...
                          return (
                            <div
                              key={key}
                              className={`${meta.bg} rounded-xl p-4 text-center border`}
                  ...
                  <div className="rounded-2xl bg-white/90 shadow-xl overflow-hidden border" style={{ borderColor: "rgba(139,92,246,0.15)" }}>
                    ...
                    <div className="p-6">
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        {paymentTotals.length === 0 && <p className="col-span-3 text-center text-purple-300 py-6 text-sm">No payment data</p>}
                      </div>
                    </div>
                  </div>
                </>
