...
export async function POST(request: NextRequest) {
  try {
    ...
    // Log reservation information for debugging
    ...
    if (!validateSMTPConfig()) {
      console.error("⚠️ Email configuration incomplete - skipping email notifications")
      return NextResponse.json(
        {
          ...data,
          emailWarning: "Reservation created but email notifications could not be sent due to missing configuration",
        },
        { status: 201 },
      )
    }
    ...
    try {
        ...
        return NextResponse.json(
          {
            ...
            message: "Reservation created but some emails failed to send",
          },
          { status: 201 },
        )
      }
      ...
      return NextResponse.json(
        {
          ...data,
          emailError: "Reservation created but emails could not be sent",
